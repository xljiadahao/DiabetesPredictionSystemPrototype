...
def getRawData():
    ...
    while (line != ""):
        ...
        results.append((paraArray[0], paraArray[1], paraArray[2], paraArray[3], paraArray[4], paraArray[5], paraArray[6], paraArray[7], paraArray[8]))
        line = f.readline()
    f.close()
    print "getRawData() done"
    return(results)
...
def process():
    f = open("neuralnetwork/templates/doc/Diabetes.csv", "r")
    w = open("neuralnetwork/templates/doc/Diabetes_Processed.csv", "w")
    results = []
    line = f.readline()
    while (line != ""):
        paraArray = line.split(",")
        if int(paraArray[0]) > 13 or float(paraArray[3]) > 63: 
            print "delete: " + line[0:len(line)-1]
            results.append((paraArray[0], paraArray[1], paraArray[2], paraArray[3], paraArray[4], paraArray[5], paraArray[6], paraArray[7], paraArray[8]))
            line = f.readline()
            continue
...
def getDataProcessed():
    f = open("neuralnetwork/templates/doc/Diabetes_Processed.csv", "r")
    results = []
    line = f.readline()
    while (line != ""):
        paraArray = line.split(",")
